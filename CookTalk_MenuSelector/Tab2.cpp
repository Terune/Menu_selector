...
void CTab2::OnNMClickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	case 0:
		...
		m_RecipeList.InsertItem(0,TEXT("감자는 삶아 으깨어서 준비합니다."));
		...
		m_RecipeList.InsertItem(2,TEXT("팬에 밀가루, 버터를 넣어 볶은 후 우유를 조금씩 넣어 가며 밀가루 멍울을 풀고 소금, 후추로 간하여 소스를 만듭니다. 이때 간을 약간 세게 하고 소스는 약간 묽게 합니다."));
		...
		m_RecipeList.InsertItem(3,TEXT("화이트소스 이분의 삼 분량에 볶은 재료와 감자를 넣어 섞습니다."));

		m_RecipeList.InsertItem(4,TEXT("그라탱접시, 지름 23센티 정도의 내열접시에 소스를 버무린 재료를 담고 남은 소스를 끼얹은 후 다진 치즈, 빵가루, 파슬리 순으로 올린다."));
		
		m_RecipeList.InsertItem(5,TEXT("법랑접시에 석쇠를 놓고 그라탱 접시를 올린 후 자동요리오븐에서 2단에서 구위냅니다.이를 위한 오븐셋팅을 하시겠습니까?"));
	...
	case 1:
		...
		m_RecipeList.InsertItem(0,TEXT("고구마는 너무 크지 않는 것으로 골라 깨끗이 씻은 후 물기를 닦아 껍질을 포크로 몇차례 찔러 줍니다."));
		m_RecipeList.InsertItem(1,TEXT("1단에 법랑접시 위에 석쇠를 놓고 고구마를 가지런히 올린 후 자동요리오븐에서 군고무마를 선택한 후 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	case 2:
		m_MaterialList.InsertItem(0,TEXT("냉동피자"));
		m_MaterialList.SetItem(0, 1, LVIF_TEXT, _T("0.2~ 0.6kg"), 0, 0, 0, NULL );
		
		m_RecipeList.InsertItem(0,TEXT("법랑접시를 석쇠에 놓고 포장을 제거한 냉동피자를 올린 후, 3단에 넣고, 자동요리오븐에서 냉동피자를 선택한 후 구워냅니다.이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	case 3:
		m_MaterialList.InsertItem(0,TEXT("식은밥"));
		m_MaterialList.SetItem(0, 1, LVIF_TEXT, _T("0.2kg"), 0, 0, 0, NULL );
		
		m_RecipeList.InsertItem(0,TEXT("상온에 보관된 식은 밥 이백그람을 구이전용 팬 위에 얇게 전체로 펴 줍니다."));
		m_RecipeList.InsertItem(1,TEXT("자동요리오븐에서 누룽지를 선택한 후 4단에서 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	case 4:
		...
		m_RecipeList.InsertItem(1,TEXT("닭다리에 올리브유를 바른 후 구이전용 팬 위에 가지런히 놓고, 4단에 넣은 뒤, 자동요리오븐에서 닭다리를 선택한 후 분량을 선택하여 구워냅니다.이를 위한 오븐셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(2,TEXT("도중에 멜로디가 울리면 한번 뒤집어줍니다."));
		break;
	case 5:
		...
		m_RecipeList.InsertItem(0,TEXT("토핑을 다음과 같이 준비합니다. 먼저 양파, 청, 홍피망, 양송이는 적당한 크기롤 썰어서 내열용기에 담고 랩을 씌워 수동요리레인지 기능으로 2~3분 가열한 후 물기를 없앱니다. 베이컨, 햄은 적당한 크기로 자릅니다. 모짜렐라 치즈는 잘게 다지고, 올리브는 모양대로 얇게 썰어줍니다. 알람설정을 하시겠습니까?"));
		m_RecipeList.InsertItem(1,TEXT("또띠아 2장을 겹쳐 구이전용 팬에 올립니다."));
		m_RecipeList.InsertItem(2,TEXT("가운데 부분에 피자소스를 고루 펴 바르고 햄, 베이컨, 양송이, 양파, 피망 순으로 토핑합니다."));
		m_RecipeList.InsertItem(3,TEXT("모짜렐라 치즈를 고루 뿌린 뒤 올리브로 장식하여 4단에 넣고 자동요리 오븐에서 또띠아 피자를 선택한후 구워냅니다. 오븐셋팅을 하시겠습니까?"));
		break;
	case 6:
		...
		m_RecipeList.InsertItem(1,TEXT("설탕은 적당히 반으로 나눕니다."));
		m_RecipeList.InsertItem(2,TEXT("볼을 2개 준비하여 한쪽은 흰자, 다른 한 쪽은 노른자를 분리해 놓고 노른자 1개는 따로 빼둡니다."));
		m_RecipeList.InsertItem(3,TEXT("노른자를 풀어 소금, 설탕, 물엿, 우유를 함께 넣고 아이보리색이 되도록 거품을 냅니다."));
		m_RecipeList.InsertItem(4,TEXT("흰자는 어느 정도 거품을 올린 후 설탕을 조금씩 나누어 넣으며 거품을 빳빳하게 내줍니다."));
		m_RecipeList.InsertItem(5,TEXT("노른자에 흰자를 삼분의 일정도 넣고 가볍게 섞다가 체에 친 밀가루를 넣어 주걱으로 가볍고 빠르게 섞어줍니다."));
		m_RecipeList.InsertItem(6,TEXT("식용유를 넣어 고루 섞은 다음 나머지 흰자 거품을 넣어 거품이 가라앉지 않도록 재빨리 섞어 줍니다."));
		m_RecipeList.InsertItem(7,TEXT("법랑접시에 유산지를 깔고 반죽을 평평하게 부어줍니다."));
		m_RecipeList.InsertItem(8,TEXT("노른자 1개에 소금을 약간 넣어 잘 풀어 비닐봉지에 담고 구멍을 작게 뚫어 반죽 위에 흘리면서 지그재그로 무늬를 만들어 줍니다."));
		m_RecipeList.InsertItem(9,TEXT("무늬 반대편을 젓가락으로 다시 지그재그로 휘저어 물결 무늬를 만듭니다."));
		m_RecipeList.InsertItem(10,TEXT("2단에 넣고 자동요리오븐에서 롤케이크를 선택하여 구워냅니다. 오븐셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(11,TEXT("젖은 면보를 준비하여 바닥에 깔고 구워낸 케이크를 바닥이 위로 오도록 엎어 줍니다."));
		m_RecipeList.InsertItem(12,TEXT("케이크 위에 잼을 고루 바른 뒤 면보 아래로 방망이를 대고 케이크를 밀어 가며 말아줍니다."));
		break;
	case 7:
		...
		m_RecipeList.InsertItem(1,TEXT("물기를 제거하고 레몬 즙, 소금, 후추로 밑간을 합니다."));
		m_RecipeList.InsertItem(2,TEXT("법랑접시 위에 석쇠를 놓고 립을 올린 후 자동요리 오븐에서 립 바비큐를 선택한 후 분량을 선택하여 구워줍니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(3,TEXT("분량대로 섞어 소스를 만듭니다."));
		m_RecipeList.InsertItem(4,TEXT("도중에 멜로디가 울리면 소스를 붓으로 고루 바른 후 남은 시간 동안 더 구워줍니다."));
		m_RecipeList.InsertItem(5,TEXT("립을 접시에 담고 소스를 한번 더 끼얹어 냅니다."));
		break;
	...
	case 8:
		...
		m_RecipeList.InsertItem(0,TEXT("상온 버터와 다진 마늘, 다진 파슬리를 섞어 식빵의 한 면에 바릅니다."));
		m_RecipeList.InsertItem(1,TEXT("법량접시에 유산지를 깔고 식빵을 올린 후 <자동요리 토스트>에서 <마늘빵(1)>을 선택한 후 분량을 선택하여 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	...
	case 9:
		...
		m_RecipeList.InsertItem(0,TEXT("베이글을 반으로 자릅니다."));
		m_RecipeList.InsertItem(1,TEXT("베이글의 안쪽 면이 위로 오게하여 구이전용 팬에 겹치지 않게 올린 후 <자동요리 토스트>에서 <베이글(2)>을 선택한 후 분략을 선택하여 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	...
	case 10:
		...
		m_RecipeList.InsertItem(1,TEXT("미리 섞은 달걀과 우유를 1에 넣고 자르듯 고루 섞습니다."));
		m_RecipeList.InsertItem(2,TEXT("마른 가루가 보이지 않고, 반죽이 하나로 뭉쳐지면 랩을 덮어 냉장고에서 30분정도 숙성 시킵니다. 이를 위한 알림셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(3,TEXT("반죽을 약 2.5센치 두께가 되도록 밀대로 민 다음 같은 크기로 자릅니다. 이때 덧가루를 사용해야 밀어 펴기가 편리합니다."));
		m_RecipeList.InsertItem(4,TEXT("법랑 접시에 유산지를 깔고 스콘을 올린 후 표면에 달걀물을 칠합니다."));
		m_RecipeList.InsertItem(5,TEXT("2단에 넣고 자동요리 오븐에서 스콘을 선택한 후 구워냅니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		break;
	...
	case 11:
		...
		m_RecipeList.InsertItem(0,TEXT("박력분은 체쳐 둡니다. 체치기 영상을 보시겠습니까?"));
		m_RecipeList.InsertItem(1,TEXT("케이크 틀 바닥과 옆면에 종이를 깔아 줍니다."));
		m_RecipeList.InsertItem(2,TEXT("버터를 우유에 함께 녹여 준 후 바닐라 오일을 한 방울 섞어 줍니다."));
		m_RecipeList.InsertItem(3,TEXT("달걀을 따뜻한 물에서 중탕한 후 볼에 넣고 풀어줍니다."));
		m_RecipeList.InsertItem(4,TEXT("달걀과 설탕을 한꺼번에 넣고 거품기를 들어보아 끝이 뾰족하고 살짝 숙여질 때까지 거품을 냅니다."));
		m_RecipeList.InsertItem(5,TEXT("거품낸 달걀에 박력분을 넣고 골고루 섞어 줍니다."));
		m_RecipeList.InsertItem(6,TEXT("위 반죽의 일부를 버터 녹인 우유에 넣어 섞은 후 다시 반죽에 넣어 재빨리 섞어 줍니다."));
		m_RecipeList.InsertItem(7,TEXT("반죽을 틀에 붓고 틀을 한번 들었다가 바닥에 놓아 반죽 속의 기포를 고르게 해 줍니다."));
		m_RecipeList.InsertItem(8,TEXT("법랑접시 위에 석쇠를 놓고 케이크 틀을 올린 후 1단에 놓고 자동요리 오븐에서 스폰지 케이크를 선택한 후 구워냅니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		break;
	case 12:
		m_MaterialList.InsertItem(0,TEXT("식빵"));
		m_MaterialList.SetItem(0, 1, LVIF_TEXT, _T("2~4개"), 0, 0, 0, NULL );

		m_RecipeList.InsertItem(0,TEXT("구이전용 팬에 식빵을 겹치지 않게 올린 후 <자동요리 토스트>에서 <식빵(3)>을 선택한 후 분량을 선택하여 구워낸다. 이를 위한 오븐셋팅을 하시겠습니까?"));
	...
	case 13:
		...
		m_RecipeList.InsertItem(1,TEXT("밤은 껍질을 벗겨 4등분하고, 대추는 씨를 빼내어 2등분하고, 잣은 고깔을 떼어냅니다."));
		m_RecipeList.InsertItem(2,TEXT("법랑 접시에 석쇠를 놓고 2리터 정도 크기의 내열 용기를 올린 후 뚜껑 또는 알루미늄 호일로 덮어 자동요리오븐에서 약신을 선택한 후 가열합니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(3,TEXT("가열이 끝나면 오븐에 약 10분 정도 그대로 두어 뜸 들인 후 꺼내어 참기름을 넣고 고루 섞어 줍니다. 이를 위한 알람 셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(4,TEXT("잣이 완전히 익을 때까지 가열합니다."));
		m_RecipeList.InsertItem(5,TEXT("오븐에서 꺼낸 다음 잘 식힙니다."));
		m_RecipeList.InsertItem(6,TEXT("약간 따뜻한 밥을 공모양으로 만든 다음 접시에 내놓습니다."));
		break;
	...
	case 14:
		...
		m_RecipeList.InsertItem(1,TEXT("밤은 껍질을 벗겨 2등분하고, 잣은 고깔을 떼어 놓습니다."));
		m_RecipeList.InsertItem(2,TEXT("대추는 씨를 빼고 2등분하고, 콩은 6시간 이상 불려서 준비합니다."));
		m_RecipeList.InsertItem(3,TEXT("내열용기에 불린 쌀, 콩, 밤, 은행, 대추, 잣, 소금을 넣고 물을 붓습니다"));
		m_RecipeList.InsertItem(4,TEXT("법랑접시에 석쇠를 놓고 2리터정도 크기의 내열용기를 올린 후 뚜껑 또는 알루미늄 호일로 덮어 1단에 넣고 자동요리오븐에서 영양밥을 선택한 후 가열합니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
	...
	case 15:
		...
		m_RecipeList.InsertItem(0,TEXT("끓는 소금물에 스파게티면을 넣고 삶아 물기 없이 건집니다."));
		m_RecipeList.InsertItem(1,TEXT("가지는 0.5센티 두께로 둥글게 썰어 소금, 후추, 올리브유를 약간씩 넣고 버무립니다."));
		m_RecipeList.InsertItem(2,TEXT("팬에 가지를 살짝 볶아 접시에 옮겨 놓고, 올리브유를 팬에 살짝 두르고 다진 마늘과 양파를 넣어 볶아 준 후 갈은 쇠고기, 소금, 후추를 넣고 볶아 양송이, 당근, 샐러리 다진 것을 넣어 익힙니다."));
		m_RecipeList.InsertItem(3,TEXT("3에 토마토 페이스트와 케첩을 넣고 껍질과 씨를 제거한 토마토를 넣어 준 후 설탕, 핫소스, 월계수 잎, 물을 넣어 끓이면서 소금, 후추로 간을 하고 마지막에 볶아 놓은 가지와 오레가노를 넣어 걸쭉한 상태로 만듭니다."));
		m_RecipeList.InsertItem(4,TEXT("삶아 놓은 스파게티면을 4에 넣고 버무려 내열접시에 담고 다진 모짜렐라 치즈와 파마산 치즈를 얹어 송송 썬 실파를 뿌립니다."));
		m_RecipeList.InsertItem(5,TEXT("법랑접시에 석쇠를 놓고 내열접시를 올린 후 자동요리오븐에서 오븐스파게티를 선택한 후 2단에서 구워냅니다. 이를위한 오븐 셋팅을 하시겠습니까?"));
		break;
	case 16:
		m_MaterialList.InsertItem(0,TEXT("박력분"));
		...
		m_RecipeList.InsertItem(1,TEXT("실온에 둔 버터를 잘 풀어 크림 상태를 만듭니다."));
		m_RecipeList.InsertItem(2,TEXT("설탕을 넣고 녹을 때까지 잘 섞어줍니다."));
		m_RecipeList.InsertItem(3,TEXT("달걀을 풀어 조금씩 나누어 넣으며 버터와 분리되지 않도록 재빨리 섞어줍니다."));
		m_RecipeList.InsertItem(4,TEXT("체친 가루와 초코칩을 넣어 반죽한 후 기름 바른 법랑접시와 구이전용 팬에 적당한 간격을 두고 반 수저씩 놓은 다음 포크로 가볍게 눌러 줍니다."));
		m_RecipeList.InsertItem(5,TEXT("법랑접시는 1단에, 구이전용 팬은 4단에 각각 넣습니다."));
		m_RecipeList.InsertItem(6,TEXT("자동요리오븐에서 초코칩 쿠키를 선택한 후 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	case 17:
		...
		m_RecipeList.InsertItem(0,TEXT("크림치즈를 상온에 두어 부드러운 상태가 되도록 한 뒤 거품기를 이용하여 잘 풀어줍니다. 달걀도 미리 풀어둡니다."));
		m_RecipeList.InsertItem(1,TEXT("1의 크림치즈에 설탕과 달걀을 조금씩 나누어 넣어가며 거품기로 섞습니다."));
		m_RecipeList.InsertItem(2,TEXT("부드럽게 풀린 반죽에 박력분과 생크림을 넣고 주걱으로 고루 섞습니다."));
		m_RecipeList.InsertItem(3,TEXT("케이크 틀에 유산지를 깔고 바닥에 카스텔라 시트를 놓은 다음 3의 반죽을 붓습니다"));
		m_RecipeList.InsertItem(4,TEXT("법랑접시에 석쇠를 놓고 케이크틀을 올린 후 1단에 넣고 자동요리오븐에서 치즈케이크를 선택한 후 구워냅니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		break;
	case 18:
		m_MaterialList.InsertItem(0,TEXT("감자"));
		m_MaterialList.SetItem(0, 1, LVIF_TEXT, _T("0.4~1.2kg"), 0, 0, 0, NULL );

		m_RecipeList.InsertItem(0,TEXT("감자를 모양이 둥글고 적당한 크기로 골라 깨끗이 씻은 후 물기를 닦아, 껍질을 포크로 몇차례 찔러 줍니다."));
		m_RecipeList.InsertItem(1,TEXT("법랑접시 위에 석쇠를 놓고 감자를 가지런히 올린 후 자동요리오븐에서 통감자를 선택한 후 구워냅니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		break;
	case 19:
		...
		m_RecipeList.InsertItem(1,TEXT("닭 껍질에 올리브유를 바릅니다."));
		m_RecipeList.InsertItem(2,TEXT("법량접시에 석쇠를 놓고 닭의 가슴부분이 아래로 가도록 하여 자동요리오븐에서 통닭을 선택한 후 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(3,TEXT("도중에 멜로디가 울리면 한번 뒤집어줍니다."));
		break;
	case 20:
		m_MaterialList.InsertItem(0,TEXT("통삼겹살"));
		m_MaterialList.SetItem(0, 1, LVIF_TEXT, _T("1.0~1.5kg"), 0, 0, 0, NULL );

		m_RecipeList.InsertItem(0,TEXT("삼겹살은 통째로 구입하여 2~3개의 덩어리로 자른 후 칼집을 내고 소금, 후추, 올리브유를 적당량 뿌립니다."));
		m_RecipeList.InsertItem(1,TEXT("법랑접시 위에 석쇠를 놓고 통삼겹살의 껍질부분이 아래로 가도록 하여 자동요리오븐에서 통삼겹살을 선택한 후 분량을 선택하여 1단에서 구워줍니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	case 21:
		...
		m_RecipeList.InsertItem(0,TEXT("크러스트는 볼에 피자 빵가루와 물을 넣고 반죽기로 표면이 매끈해지도록 치댄 후, 반죽을 밀어서 구이전용 팬에 올립니다."));
		m_RecipeList.InsertItem(1,TEXT("소스는 팬을 달구어 다진 마늘, 양파를 넣고 볶은 후 다진 쇠고기를 넣고 볶은 후, 토마토페이스트를 넣고 잘 섞은 후 육수를 붓고 월계수 잎을 넣어 걸쭉한 상태로 졸이고 오레가노를 넣어 잘 섞어줍니다. 마지막에 소금, 후추로 간합니다."));
		m_RecipeList.InsertItem(2,TEXT("토핑은 양파, 피망, 양송이는 적당한 크기로 썰어서 살짝 데쳐서 물기를 없앤 후, 베이컨, 햄은 적당한 크기로 자릅니다. 그리고, 모짜렐라 치즈는 잘게 다지고 올리브는 모양대로 얇게 썰어 둡니다."));
		m_RecipeList.InsertItem(3,TEXT("1의 크러스트 가장자리 1센치 정도만 남기고 가운데 부분은 포크로 찔러주고, 가장자리 1센치 남긴 부분에 올리브유를 살짝 발라 줍니다."));
		m_RecipeList.InsertItem(4,TEXT("크러스트 가운데 부분의 2의 소스를 고구 펴 바르고 모짜렐라 치즈 50그램을 얇게 뿌립니다."));
		m_RecipeList.InsertItem(5,TEXT("햄, 베이컨, 양송이, 양파, 피망 순으로 토핑합니다."));
		m_RecipeList.InsertItem(6,TEXT("마지막으로 나머지 모짜렐라 치즈를 고루 뿌린 뒤 올리브로 장식하여 4단에 넣고 자동요리오븐에서 피자를 선택한 후 구워냅니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		break;
	case 22:
		m_MaterialList.InsertItem(0,TEXT("식빵"));
		m_MaterialList.SetItem(0, 1, LVIF_TEXT, _T("2~4개"), 0, 0, 0, NULL );
		...
		m_RecipeList.InsertItem(0,TEXT("토핑은 다음과 같이 만듭니다.양파,청,홍피망,양송이는 적당한 크기로 썰어서 내열용기에 담고 랩을 씌워 <수동요리레인지> 기능으로 2~3분 가열한 후 물기를 없앱니다. 2. 베이컨, 햄은 적당한 크기로 자릅니다. 3. 모짜렐라 치즈는 잘게 다지고 올리브는 모양대로 얇게 썰어 둡니다."));
		m_RecipeList.InsertItem(1,TEXT("상온 버터를 식빵의 한 면에 바른 후, 피자소스를 바릅니다."));
		m_RecipeList.InsertItem(2,TEXT("햄, 베이컨, 양송이, 양파, 피망 순으로 토핑합니다."));
		m_RecipeList.InsertItem(3,TEXT("모짜렐라 치즈를 고루 뿌린 뒤 올리브로 장식합니다."));
		m_RecipeList.InsertItem(4,TEXT("법량접시에 유산지를 깔고 식빵을 올린 후 <자동요리 토스트>에서 <피자토스트(4)>를 선택한 후 구워냅니다. 이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	case 23:
		...
		m_RecipeList.InsertItem(0,TEXT("팬을 달구어 다진 마늘, 양파를 넣고 볶은 후 한김 식힙니다."));
		m_RecipeList.InsertItem(1,TEXT("다진 쇠고기, 돼지고기에 다진 마늘, 양파, 달걀, 우유, 빵가루, 소금, 후추, 분유를 넣고 끈기가 생길 때까지 힘껏 치대 줍니다."));
		m_RecipeList.InsertItem(2,TEXT("끈기가 생긴 고기 반죽을 4등분하여 1.5센치 두께, 지름 8센치로 동글 납작하게 만들어 가운데 부분을 오목하게 눌러 줍니다."));
		m_RecipeList.InsertItem(3,TEXT("구이전용 팬에 올리브유를 적당히 두르고 가지런히 올린 후 자동요리오븐에서 햄버그 스테이크를 선택한 후 구워냅니다. 이를 위한 오븐 셋팅을 하시겠습니까?"));
		m_RecipeList.InsertItem(4,TEXT("도중에 멜로디가 울리면 한번 뒤집어 줍니다."));
		m_RecipeList.InsertItem(5,TEXT("팬을 달구어 다진 마늘, 양파를 넣고 볶은 후 케첩을 넣어 고루 섞이도록 볶아 줍니다. 우스터 소스, 적포도주, 육수를 넣은 후 이분의 일로 될때까지 졸입니다."));
		m_RecipeList.InsertItem(6,TEXT("우유를 조금씩 넣어 가며 섞어준 후 소금, 후추로 간하여 웨지감자 체에 걸러 햄버그 스테이크에 곁들여 냅니다."));
		break;
	case 24:
		m_MaterialList.InsertItem(0,TEXT("식빵"));
		...
		m_RecipeList.InsertItem(0,TEXT("통 식빵을 반으로 자릅니다."));
		m_RecipeList.InsertItem(1,TEXT("식빵의 한쪽 면이 위로 오게 하여 밑바닥에 구멍이 나지 않게 엑스자 칼집을 넣어줍니다."));
		m_RecipeList.InsertItem(2,TEXT("칼집을 낸 부분에 꿀을 넣고 식빵 뒷면에 버터를 바릅니다."));
		m_RecipeList.InsertItem(3,TEXT("법량접시에 유산지를 깔고 식빵을 올린 후 <자동요리 토스트>에서 <허니브레드(5)> 선택한 후 구워냅니다.  이를 위한 오븐셋팅을 하시겠습니까?"));
		break;
	}
